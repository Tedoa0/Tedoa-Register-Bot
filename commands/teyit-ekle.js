...
if (member.id === message.author.id) return message.channel.send(embed.setDescription(`${message.author} Kendinine kayıt ekleyemezsin!`)).then(tedoa => tedoa.delete({timeout : 5000})).then(message.react(emoji.redemoj))
...
if(!args[1]) {
    ...
    .teyitekle @Tedoa/ID erkek <miktar>
    .teyitekle @Tedoa/ID kadın <miktar>`)
...
if(args[1] === 'erkek') {
let miktar = args[2]
if(!miktar) return message.channel.send(embed.setDescription(`eklemek istediğinz kadar sayı girin!`)).then(tedoa => tedoa.delete({timeout : 7000})).then(message.react(emoji.redemoj)) 
...
if(args[1] === 'kadın') {
    ...
    exports.help = {
        name : 'kayıt-ekle',
        help: "kayıt-ekle [tedoa/ID] [erkek/kız] [miktar]",
        cooldown: 0
     }
